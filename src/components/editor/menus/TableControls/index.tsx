...
export function TableControls() {
  ...
  const triggerCls = (open: boolean, isDragging?: boolean) =>
    cn(TRIGGER_CLS, (open || isDragging) && "opacity-100 pointer-events-auto");
      ...
      {createPortal(
        <>
          ...
          >
            <div
              className={cn(
                ...
                (colOpen || drag?.isDragging) && "opacity-0",
              )}
            />
            <DropdownMenu
              open={colOpen}
            ...
            >
              <DropdownMenuTrigger
                className={triggerCls(colOpen, drag?.isDragging)}
                style={{
                  width: BTN,
                  height: BTN / 2,
          ...
          >
            ...
            >
              <DropdownMenuTrigger
                className={triggerCls(rowOpen, drag?.isDragging) + " rotate-90"}
          ...
          <div className="group absolute z-40" style={{ ...cell, width: BTN }}>
            ...
            <DropdownMenu open={cellOpen} onOpenChange={setCellOpen}>
              <DropdownMenuTrigger
                className={
                  triggerCls(cellOpen, drag?.isDragging) + " rotate-90"
                }
        ...
        focused.tableWrapper.firstChild as HTMLElement,
      )}

      {createPortal(
        <TableEdgeAddons tablePos={focused.tablePos} />,
        focused.tableWrapper,
      )}
    </>
  );
}
